...
CREATE TABLE IF NOT EXISTS `lental-payment` (
...
CREATE TABLE IF NOT EXISTS `lental-order` (
    `lental_id` Long NOT NULL,
    `order_id` BINARY(16) NOT NULL,
    `quantity` Int NULL,
    `price` int NULL,
...
ALTER TABLE `lental` ADD CONSTRAINT `PK_LENTAL` PRIMARY KEY (`id`);
ALTER TABLE `product_image` ADD CONSTRAINT `PK_PRODUCT_IMAGE` PRIMARY KEY (`id`);
ALTER TABLE `product-order` ADD CONSTRAINT `PK_PRODUCT-ORDER` PRIMARY KEY (`product_id`, `order_id`);
ALTER TABLE `lental-payment` ADD CONSTRAINT `PK_LENTAL-PAYMENT` PRIMARY KEY (`lental_id`, `payment_id`);
ALTER TABLE `product_payment` ADD CONSTRAINT `PK_PRODUCT_PAYMENT` PRIMARY KEY (`product_id`, `payment_id`);
ALTER TABLE `board_image` ADD CONSTRAINT `PK_BOARD_IMAGE` PRIMARY KEY (`id`);
ALTER TABLE `lental-order` ADD CONSTRAINT `PK_LENTAL-ORDER` PRIMARY KEY (`lental_id`, `order_id`);
...
ALTER TABLE `lental` ADD CONSTRAINT `FK_product_TO_lental_1` FOREIGN KEY (`product_id`) REFERENCES `product` (`id`);
ALTER TABLE `product_image` ADD CONSTRAINT `FK_product_TO_product_image_1` FOREIGN KEY (`product_id`) REFERENCES `product` (`id`);
ALTER TABLE `product-order` ADD CONSTRAINT `FK_product_TO_product-order_1` FOREIGN KEY (`product_id`) REFERENCES `product` (`id`);
ALTER TABLE `product-order` ADD CONSTRAINT `FK_Order_TO_product-order_1` FOREIGN KEY (`order_id`) REFERENCES `Order` (`id`);
ALTER TABLE `lental-payment` ADD CONSTRAINT `FK_lental_TO_lental-payment_1` FOREIGN KEY (`lental_id`) REFERENCES `lental` (`id`);
ALTER TABLE `lental-payment` ADD CONSTRAINT `FK_Payment_TO_lental-payment_1` FOREIGN KEY (`payment_id`) REFERENCES `Payment` (`id`);
...
ALTER TABLE `lental-order` ADD CONSTRAINT `FK_lental_TO_lental-order_1` FOREIGN KEY (`lental_id`) REFERENCES `lental` (`id`);
ALTER TABLE `lental-order` ADD CONSTRAINT `FK_Order_TO_lental-order_1` FOREIGN KEY (`order_id`) REFERENCES `Order` (`id`);
...
ALTER TABLE `Order` ADD CONSTRAINT `FK_member_TO_Order_1` FOREIGN KEY (`member_id`) REFERENCES `member` (`id`);
